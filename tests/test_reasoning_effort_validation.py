...
def test_the_floors_astra_dropped_are_rejected(effort):
    """gpt-6-astra kept neither of the two lowest gpt-5 efforts, so "low" is as cheap as it gets."""
    with pytest.raises(ValueError, match=effort):
        validate_reasoning_effort_for_model("gpt-6-astra", effort)


def test_the_max_effort_astra_added_is_accepted():
    validate_reasoning_effort_for_model("gpt-6-astra", "max")


def test_the_max_effort_is_not_opened_up_to_older_models():
    """ "max" became a ReasoningEffort member for gpt-6-astra; the gpt-5 line still tops out at xhigh."""
