...
class LiteLLM(BaseLLM):
    def __init__(self, max_tokens=100, buffer_size=40, model="gpt-3.5-turbo-16k", temperature=0.1, language=DEFAULT_LANGUAGE_CODE, **kwargs):
        ...
        logger.info(f"Initializing LiteLLM with model: {self.model} and max tokens {max_tokens}")
        ...
        if self.assistant_id:
            logger.info(f"Initializing LiteLLM assistant with assistant id {self.assistant_id}")
    ...
    async def generate_stream(self, messages, synthesize=True, request_json=False, meta_info=None):
        ...
        model_args = {
            **self.model_args,
            ...
            "stream": True,
            "stop": ["User:"],
            "user": f"{self.run_id}#{meta_info['turn_id']}" if meta_info and self.run_id else None
        }
    ...
    async def generate(self, messages, request_json=False):
        ...
        completion = await acompletion(model=self.model, temperature=0.0, messages=messages,
