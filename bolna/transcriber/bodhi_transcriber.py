...
class BodhiTranscriber(BaseTranscriber):
            ...
            async for msg in ws:
                try:
                    ...
                    if transcript.strip() == self.curr_message.strip() and msg["type"] != "complete":
                        logger.trans(f"text {transcript} is equal to finalized_transcript {self.finalized_transcript} is equal to curr_message {self.curr_message}")
                        continue

                    elif msg["type"] == "complete":
                        # If is_final is true simply update the finalized transcript
                        logger.trans(f"Setting up is final as true")
                    ...
                    if transcript and len(transcript.strip()) == 0 or transcript == "":
                        logger.trans(f"Empty transcript")
                        if (time.time() - self.last_non_empty_transcript) * 1000 >= self.utterance_end:
                            logger.trans(f"Last word filled transcript atleast  {self.utterance_end} away from now and hence setting the finalized_transcript to null ")
                            logger.trans(
                            ...
                            logger.trans(f"Just continuing as it's an empty transcript")
                            continue

                    # Now that we know this ain't an empty transcript, we will set last_non_empty transcript
                    self.last_non_empty_transcript = time.time()                    
                    if self.finalized_transcript == "":
                        logger.trans(f"Sending transcriber_begin as finalized transcript is null")
    ...
    def connect(self):
        try:
            ...
            logger.trans(f"Connecting to {websocket_url}, with extra_headers {request_headers}")
    ...
    async def transcribe(self):
        ...
        try:
            async with self.connect() as ws:
                if self.stream:
                    logger.trans(f"Connectioin established")
                    ...
                    logger.trans(f"Sending self.config {self.config}")
                    await ws.send(
                        json.dumps(
                            {
                                "config": self.config}))
                    ...
                    async for message in self.receiver(ws):
                        if self.connection_on:
                            ...
                            logger.trans("closing the connection")
                else:
